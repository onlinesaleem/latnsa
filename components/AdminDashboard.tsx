...
export default function AdminDashboard() {
 ...
 const [reportForm, setReportForm] = useState<ReportFormData>({
  ...
  const filteredAssessments = assessments.filter(assessment => {
    const matchesSearch = 
      assessment.registrantName.toLowerCase().includes(searchTerm.toLowerCase()) ||
      assessment.registrantEmail?.toLowerCase().includes(searchTerm.toLowerCase()) ||
      (assessment.subjectName && assessment.subjectName.toLowerCase().includes(searchTerm.toLowerCase()))
                        ...
                        <div>
                          ...
                          <div className="text-sm text-gray-500">
                            {assessment.formType === 'PROXY' 
                              ? `by ${assessment.registrantName} (${assessment.relationship})`
                              : assessment.registrantEmail
                            }
                          </div>
                          {assessment.subjectAge && (
                            <div className="text-sm text-gray-500">
                              Age: {assessment.subjectAge} | {assessment.subjectGender}
                            </div>
                          )}
                        </div>
                      </td>

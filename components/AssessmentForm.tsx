...
export default function AssessmentForm({
  ...
  const [currentStep, setCurrentStep] = useState<
    "selection" | "patient-info" | "form" | "complete"
  >("selection");
  ...
  const relationshipOptions = [
    ...
    { value: "Spouse", label: { en: "Spouse", ar: "زوج/زوجة" } },
    { value: "Parent", label: { en: "Parent", ar: "والد/والدة" } },
    { value: "Sibling", label: { en: "Sibling", ar: "شقيق" } },
    { value: "Friend", label: { en: "Friend", ar: "صديق" } },
    { value: "Guardian", label: { en: "Guardian", ar: "ولي أمر" } },
  ...
  const loadQuestions = async () => {
    try {
      ...
      console.log("Loaded question groups:", data);
      if (response.ok) {
  ...
  const validatePatientInfo = (): boolean => {
    ...
    if (!patientInfo.dateOfBirth) {
      toast.error(
        isArabic ? "يرجى إدخال تاريخ الميلاد" : "Please enter date of birth"
      );
      return false;
    }
    ...
    if (formType === "SELF" && !patientInfo.email) {
      toast.error(
        isArabic ? "يرجى إدخال البريد الإلكتروني" : "Please enter email"
      );
      return false;
    }
    ...
    if (formType === "PROXY") {
      ...
      if (!proxyInfo.proxyEmail && !proxyInfo.proxyPhone) {
        toast.error(
          isArabic
            ? "يرجى إدخال بريد إلكتروني أو رقم هاتف"
  ...
  const handlePatientInfoSubmit = async () => {
    ...
    try {
      ...
      toast.error(
        isArabic ? "خطأ في إنشاء السجل الطبي" : "Error creating patient record"
      );
    } finally {
      setLoading(false);
    }
  ...
  const saveProgress = async () => {
    if (!patientId) {
      toast.error(
        isArabic
          ? "خطأ: لم يتم العثور على معلومات المريض"
  ...
  const submitAssessment = async () => {
    ...
    try {
      ...
      toast.error(
        isArabic ? "خطأ في إرسال التقييم" : "Error submitting assessment"
      );
    } finally {
      setLoading(false);
    }
  ...
  const renderQuestion = (question: Question) => {
    ...
    if (typeof question.options === "string") {
      ...
      parsedOptions = question.options as {
        english: string[];
        arabic: string[];
      };
    }
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <input
                  ...
                  className="w-full px-3 py-2 border border-gray-300 rounded-md focus:ring-blue-500 focus:border-blue-500"
                  placeholder={
                    isArabic ? "email@example.com" : "email@example.com"
                  }
              ...
              <div>
                ...
                <input
                  ...
                  className="w-full px-3 py-2 border border-gray-300 rounded-md focus:ring-blue-500 focus:border-blue-500"
                  placeholder={
                    isArabic ? "+966 5X XXX XXXX" : "+966 5X XXX XXXX"
                  }
                />
              </div>
            </div>
  ...
  // Filter questions based on applicableFor field
  const applicableQuestions = currentGroup.questions.filter((question) => {
    // If applicableFor is not defined or empty, show the question
    if (!question.applicableFor || question.applicableFor.length === 0) {
      return true;
    }
    // Check if current formType is in the applicableFor array
    return question.applicableFor.includes(formType);
  });
    ...
    <div className={`max-w-4xl mx-auto p-6 ${isArabic ? "rtl" : "ltr"}`}>
      ...
      {patientMRN && (
        <div className="bg-blue-50 border border-blue-200 rounded-lg p-3 mb-4 flex justify-between items-center">
          ...
          <div className="text-sm text-gray-600">{patientInfo.fullName}</div>
        </div>
      )}
      ...
      <div className="mb-8">
        ...
        <div className="w-full bg-gray-200 rounded-full h-2">
          <div
            ...
            style={{
              width: `${
                ((currentGroupIndex + 1) / questionGroups.length) * 100
              }%`,
            }}
          ></div>
        </div>
      ...
      <div className="bg-white rounded-lg shadow-md p-6 mb-6">
        {applicableQuestions.length > 0 ? (
          ...
          <div className="text-center py-8 text-gray-500">
            {isArabic
              ? "لا توجد أسئلة متاحة لهذا القسم"
